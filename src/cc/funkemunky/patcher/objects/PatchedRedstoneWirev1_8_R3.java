...
public class PatchedRedstoneWirev1_8_R3 extends BlockRedstoneWire implements RedstoneWire {
    ...
    static {
        ...
        facings = (EnumDirection[]) ArrayUtils.addAll(PatchedRedstoneWirev1_8_R3.facingsVertical, (Object[]) PatchedRedstoneWirev1_8_R3.facingsHorizontal);
        final Set<BaseBlockPosition> set = Sets.newLinkedHashSet();
        for (final EnumDirection facing : PatchedRedstoneWirev1_8_R3.facings) {
            set.add( ReflectUtil.getOfT(facing, BaseBlockPosition.class));
        }
        for (final EnumDirection facing2 : PatchedRedstoneWirev1_8_R3.facings) {
            final BaseBlockPosition v1 =  ReflectUtil.getOfT(facing2, BaseBlockPosition.class);
            for (final EnumDirection facing3 : PatchedRedstoneWirev1_8_R3.facings) {
    ...
    public PatchedRedstoneWirev1_8_R3() {
    ...
    private void calculateCurrentChanges(final World world, final BlockPosition blockposition) {
        ...
        while (!this.turnOff.isEmpty()) {
            ...
            final int oldPower = state.get(PatchedRedstoneWirev1_8_R3.POWER);
            this.g = false;
    ...
    private void addWireToList(final World worldIn, final BlockPosition pos, final int otherPower) {
        ...
        if (state.getBlock() == this) {
            final int power = state.get(PatchedRedstoneWirev1_8_R3.POWER);
    ...
    private void checkSurroundingWires(final World worldIn, final BlockPosition pos) {
        ...
        int ownPower = 0;
        if (state.getBlock() == this) {
            ownPower = state.get(PatchedRedstoneWirev1_8_R3.POWER);
        }
        for (final EnumDirection facing : PatchedRedstoneWirev1_8_R3.facingsHorizontal) {
            final BlockPosition offsetPos = pos.shift(facing);
            if (facing.k().c()) {
                this.addWireToList(worldIn, offsetPos, ownPower);
            }
        }
        for (final EnumDirection facingVertical : PatchedRedstoneWirev1_8_R3.facingsVertical) {
            final BlockPosition offsetPos = pos.shift(facingVertical);
            final boolean solidBlock = worldIn.getType(offsetPos).getBlock().u();
            for (final EnumDirection facingHorizontal : PatchedRedstoneWirev1_8_R3.facingsHorizontal) {
    ...
    private List<EnumDirection> getSidesToPower(final World worldIn, final BlockPosition pos) {
        ...
        if (retval.isEmpty()) {
            return Lists.newArrayList(PatchedRedstoneWirev1_8_R3.facingsHorizontal);
        }
    ...
    private void addAllSurroundingBlocks(final BlockPosition pos, final Set<BlockPosition> set) {
        for (final BaseBlockPosition vect : PatchedRedstoneWirev1_8_R3.surroundingBlocksOffset) {
            set.add(pos.a(vect));
        }
    }

    private IBlockData setWireState(final World worldIn, final BlockPosition pos, IBlockData state, final int power) {
        state = state.set(PatchedRedstoneWirev1_8_R3.POWER,  power);
